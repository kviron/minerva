...
import { documentDetailResponseSchema, documentTreeResponseSchema } from '../../../../shared/documents/contracts'
...
describe('documents reader API boundary', () => {
  it('validates recursive tree responses', () => {
    expect(documentTreeResponseSchema.parse([{
      id,
      title: 'Архитектура',
      slug: 'architecture',
    ...
    expect(() => documentTreeResponseSchema.parse([{ id, children: [] }])).toThrow()
  })
  ...
  it('validates Tiptap content recursively and rejects unknown response fields', () => {
    ...
    expect(documentDetailResponseSchema.parse(response)).toEqual(response)
    expect(() => documentDetailResponseSchema.parse({ ...response, storagePath: '/private/document.json' })).toThrow()
  })
