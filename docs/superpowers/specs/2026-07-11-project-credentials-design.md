...
`/projects/:id/credentials` renders `Данные`, `Категории`, and `Архив` tabs. The Data tab provides category filtering and debounced server-side search by credential title, visible login, dynamic-field label, and encrypted dynamic-field value under ADR 0014. Secret-capable search strings use a validated `POST` body rather than a URL query, and responses remain safe list projections without passwords or dynamic-field values. The Categories tab lists visible categories and opens permission-aware management. The Archive tab lists safe archival metadata without revealing credential values.
