...
- Credentials: view, create, update, archive; credential categories: create, update, archive, manage access.
- AI assistant: use, manage provider connection and safe limits.

### Project credentials

- Projects start without predefined credential categories. Project Admins create single-level categories and grant access to project roles and/or individual active project members.
- Every credential belongs to one category and contains a title, optional login/password, and ordered dynamic text, secret, URL, or note fields.
- Every active Project Admin can read and administer every category; this access cannot be removed by category grants.
- Other members require both the stable project permission and an allow grant through their role or membership. Explicit deny grants are not supported in the first release.
- Credential values are authenticated-encrypted before persistence. Authorized lists display login plaintext but never password plaintext. Password reveal/copy is separately authorized, rate-limited, audited, and never logged with plaintext.
- Credentials and categories are recoverably archived and are not exposed through MCP.

### Project AI assistant

- An assistant is available from every accessible project page and answers only from documentation the signed-in member is currently authorized to read.
- `project.ai.use` authorizes assistant turns. `project.ai.manage` authorizes dedicated provider connection and safe-limit administration. Stable permission codes, active account, active project, and active membership are checked server-side for every request.
- Provider keys use dedicated encrypted AI connection storage and never pass through project Credentials, browser state, MCP, logs, audit metadata, or generated indexes.
- The initial assistant boundary is read-only: no arbitrary URL access, code execution, credential access, MCP calls, or document mutations are available to the model.
- Document changes are proposals only. Pending proposal payloads are private,
  server-owned, limited to 64 KiB, and retained for 15 minutes; terminal
  receipts are content-free and retained for at most 24 hours. Minerva renders
  an exact diff, requires explicit confirmation, rechecks the existing document
  permission and optimistic revision, and invokes the shared idempotent
  application service as the user.
- Persistent assistant conversations are private to their author within one project. Visible messages and validated citations are retained for 30 days after activity; manual deletion is recoverable for 7 days before purge. Current `project.ai.use` authorization is required for every history operation.

## Projects

A project contains a name, optional description, lifecycle status, timestamps, creator, memberships, roles, documents, and audit records. Physical deletion is not part of the first lifecycle release.

### Project lifecycle

- The closed state set is `active`, `paused`, `closed`, and `archived`. New
  projects start active.
- The only transitions are `active -> paused`, `paused -> active`,
  `active|paused -> closed`, `closed -> active`, `closed -> archived`, and
  `archived -> closed`. Clients submit semantic transition commands and never
  assign status directly.
- Pause, resume, close, reopen, archive, and restore use separate stable
  permission codes. Built-in Project Admin receives all lifecycle permissions;
  custom roles receive none through migration and may be configured explicitly.
- Authorized authenticated reads, including separately authorized credential
  reveal, remain available while paused or closed. Ordinary work, content, and
  configuration mutations require an active project. Narrow access-reducing
  security operations remain available while paused or closed.
- AI and MCP project operations require an active project. A project transition
  does not revoke a resource-wide OAuth grant; every MCP operation rechecks the
  selected project's lifecycle, membership, scope, and permission.
- Existing public documentation links remain readable while paused or closed.
  Archiving atomically revokes them, and restoring the project never
  resurrects them.
- Archived projects are absent from ordinary navigation and deny ordinary
  authenticated, public, AI, and MCP access. Restore returns a project to
  closed, so reopening work is a separate authorized and audited action.
- Every accepted transition advances a monotonic lifecycle revision and writes
  immutable business history plus a separate content-free audit event.
- Project lifecycle administration remains web/API-only and is not exposed
  through MCP.
...
- Explicit draft saves use optimistic concurrency. A client submits the draft revision it edited; a stale revision returns a conflict and never silently overwrites newer work. Drafts are never saved by a timer or implicitly during navigation.
- Publishing accepts an optional change summary and creates an immutable snapshot of the title, validated Tiptap content, internal-link targets, and referenced images.
...
- Archiving a document recoverably archives its complete active descendant branch as one batch. Restoring the batch returns the hierarchy to its original active parent without changing stable IDs, slugs, drafts, or published history.
- A never-published active branch may be permanently discarded only when every page in the branch is a draft and none has an immutable version. Any branch containing current or historical published material must use recoverable archive.
- Document slugs remain project-unique across active and archived pages so that a restored page keeps its stable URL.
...
- Documents may embed allow-listed external materials through structured provider descriptors. Authors paste provider links, while Minerva validates and normalizes them; arbitrary HTML, iframe markup, scripts, URLs, and iframe attributes are never accepted. The first provider is Figma Design, prototypes, FigJam, and Figma Slides.
- Members with `documents.share` may create revocable capability links for one document or its dynamic active branch. A public link exposes only the latest immutable published snapshot of eligible pages; drafts, archived pages, private project navigation, and editing capabilities never cross the public boundary.
- Public documentation uses a separate guest layout and exactly shaped GET endpoints. Exact-document links omit branch navigation; branch links expose only currently eligible published descendants. Rotation, revocation, archival, and movement outside the branch fail closed on the next request. Public responses are `no-store`, `no-referrer`, and `noindex`; unavailable responses are content-free and never include the capability, request URL, stack, storage path, or user data.
- Project permissions apply to the entire document tree in the first release.

The initial editor supports paragraphs, headings, emphasis, lists, blockquotes, code blocks, tables, links, images, and allow-listed Figma embeds. Generic file attachments and additional reviewed embed providers can follow after these paths are proven.
...
- Credential access through MCP, bulk secret export, automatic login, and third-party secret-manager synchronization are explicitly excluded from this release.
...
- An Editor can create a page, explicitly save a draft, resolve a stale-edit conflict, and publish a version.
