...
- Protected application APIs by default with centralized route classification, explicit public identity and database-health exceptions, Better Auth pass-through, and super-administrator enforcement for administration routes while retaining handler-level session defense in depth.
