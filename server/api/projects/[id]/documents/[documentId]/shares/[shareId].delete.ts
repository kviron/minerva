...
export default defineEventHandler(async (event) => {
  ...
  const result = await getDocumentPublicShareManagementService().revoke({
    projectId: params.id,
    rootDocumentId: params.documentId,
    shareId: params.shareId,
    actorUserId: session.user.id,
  })
  return documentPublicShareProjectionSchema.parse(unwrapDocumentPublicShareResult(result))
})
