...
3. Extract pure deterministic transformations: explicit inputs, immutable outputs, no I/O or ambient state.
4. Represent repeated closed sets with an `as const` object and a derived union.
5. Inject only the narrow capabilities the use case needs; compose concrete adapters at runtime.
6. Introduce a discriminated `Result` selectively when expected domain outcomes otherwise require exception-heavy branching. Retain framework-compatible `Error` objects for unexpected failures and framework error hooks.
7. Re-run the focused tests, then the relevant suite. Do not add `fp-ts`, Effect, or another functional library unless the project already justifies it.
