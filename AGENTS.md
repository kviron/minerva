...
- Prefer contextual typing, explicit container and return types, discriminated unions, and control-flow narrowing over type assertions. Do not use non-null assertions to bypass nullability. Use `as const` only to preserve literal types and closed sets; keep any unavoidable `as SomeType` local to an external boundary and justify it with validation or a comment.
